...
function MobilePageShell({ children }: { children: React.ReactNode }) {
  ...
  const isCoach = pathname === "/calibra";
  ...
  // local state (e.g. the Coach chat messages). The animation is only applied
  // on mobile to preserve the existing page-transition feel.
  const isAnimated = isMobile && !reducedMotion;

  return (
    <motion.div
      className={`w-full ${isCoach ? "flex h-full min-h-0 flex-1 flex-col" : ""}`}
...
export default function AppGroupLayout({
  ...
  return (
    <>
      <div
        className={`bg-background ${isCoach ? "flex h-dvh overflow-hidden" : "min-h-screen"}`}>
        {!chromeHidden && <PillNav />}
        <main
          style={isCoach ? undefined : { paddingBottom: "96px" }}
