...
export default function WorkoutForm({
  ...
  const [duration, setDuration] = useState(
    initialWorkout?.duration?.toString() || "",
  );
  ...
  const [exercises, setExercises] = useState<ExState[]>(
    initialWorkout?.exercises?.map((e) => {
      ...
      if (e.sets && e.sets.length > 0) {
        mappedSets = e.sets.map((s) => ({
          id: uid(),
          reps: s.reps.toString(),
          kg: s.kg.toString(),
        }));
      } else if (e.reps && e.reps.length > 0) {
        mappedSets = e.reps.map((r) => ({
          id: uid(),
          reps: r.toString(),
          kg: (e.kg || 0).toString(),
        }));
    ...
    setExercises((prev) =>
      prev.map((e) => {
        ...
        return {
          ...
          sets: [
            ...e.sets,
            { id: uid(), reps: "", kg: lastSet ? lastSet.kg : "" },
          ],
        };
      }),
    );
        ...
        <div className="pt-4">
          ...
          <div className="space-y-4">
            {exercises.map((ex) => (
              <div
                key={ex.id}
                className="bg-subtle border border-border rounded-2xl p-4 flex flex-col gap-3">
                {/* Exercise Header */}
