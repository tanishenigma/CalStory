...
export default function FAB() {
  ...
  return (
    <div ref={rootRef} className="fixed bottom-8 right-6 z-50 flex flex-col items-end">
      {open && (
        <div className="mb-4">
      ...
      >
          <div className="relative w-6 h-6">
            <>
              <Utensils
                size={24}
                className={`absolute inset-0 transition-all duration-300 ${open ? "opacity-0 scale-75 rotate-90"
                  : "opacity-100 scale-100 rotate-0"
                  }`}
              />
              <UtensilsCrossed
                size={24}
                className={`absolute inset-0 transition-all duration-300 ${open ? "opacity-100 scale-100 rotate-0"
